...
- Secondary CLI executable `uvsm` as a short alias for `uv-script-manager`
- GitHub Actions publish workflow (`.github/workflows/publish.yml`) to build and publish tagged releases to PyPI via Trusted Publishing

### Changed

- Installation docs now show both `uv-script-manager` and `uvsm` commands
...
- Configurable installation workflow using TOML-based settings for repository/bin directories, symlink creation,
verification, and permissions.
- State tracking powered by TinyDB and Pydantic models to record script metadata, enabling idempotent operations and
update checks.
- Rich terminal experience implemented with Click and Rich, including progress indicators, tables, and considerate error
reporting.
